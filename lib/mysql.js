...
export const getUser = async (username) => {
  const queryString = 'SELECT id, email, fname, lname, username, image FROM users WHERE username = ?';
...
export const getEmail = async (email) => {
  ...
  const users = await connection.query(queryString, queryArgs);
  return users[0][0];
};
...
export const addUser = async (credentials) => {
  const { email, fname, lname, username, password, image } = credentials;
  const queryString = 'INSERT INTO users (email, fname, lname, username, password, image) VALUES ?';
  const queryArgs = [[email, fname, lname, username, password, image]];
  const data = await connection.query(queryString, [queryArgs]);
  return data[0];
};
...
export const addRoom = async (room_name, user_id, image) => {
